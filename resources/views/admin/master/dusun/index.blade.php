...
@section('breadcrumb-item', 'Dusun')
@section('page-title', 'Daftar Dusun')
...
@open-delete-modal.window="showDeleteModal = true; deleteId = $event.detail.id">
    ...
    <x-card class="overflow-hidden p-0">
        <div class="overflow-x-auto p-4">
            <table id="dusunTable" class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-200 text-sm">
                    @foreach ($dusuns as $dusun)
                        <tr>
                            <td class="px-6 py-4 text-slate-900 font-bold">{{ $dusun->nama }}</td>
                            <td class="px-6 py-4 text-right space-x-2">
                                <button @click="$dispatch('open-edit-modal', { id: '{{ $dusun->id }}', nama: '{{ addslashes($dusun->nama) }}' })" 
                                ...
                                <button @click="$dispatch('open-delete-modal', { id: '{{ $dusun->id }}' })" 
    ...
    <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60" style="display: none;">
        <div @click.outside="showCreateModal = false" class="bg-white border border-slate-200 rounded-lg p-6 max-w-md w-full">
            <h3 class="text-base font-bold text-slate-900 mb-4">Tambah Dusun Baru</h3>
            <form action="{{ route('admin.master.dusun.store') }}" method="POST" class="space-y-4">
                @csrf
                <x-input label="Nama Dusun" name="nama" placeholder="Masukkan nama dusun..." required />
    ...
    <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60" style="display: none;">
        <div @click.outside="showEditModal = false" class="bg-white border border-slate-200 rounded-lg p-6 max-w-md w-full">
            <h3 class="text-base font-bold text-slate-900 mb-4">Edit Dusun</h3>
            <form :action="'{{ url('admin/master/dusun') }}/' + editId" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <x-input label="Nama Dusun" name="nama" x-model="editNama" required />
                <div class="flex justify-end gap-2 pt-2">
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60" style="display: none;">
        <div @click.outside="showDeleteModal = false" class="bg-white border border-slate-200 rounded-lg p-6 max-w-sm w-full">
            ...
            <p class="text-sm text-slate-500 mb-6">Apakah Anda yakin ingin menghapus dusun ini?</p>
            <form :action="'{{ url('admin/master/dusun') }}/' + deleteId" method="POST">
                @csrf
                @method('DELETE')
