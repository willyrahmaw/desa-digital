...
@section('breadcrumb-item', 'Tambah')
@section('page-title', 'Tambah Penduduk Baru')
...
<x-card class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.master.penduduk.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-input label="NIK (Nomor Induk Kependudukan)" name="nik" placeholder="Masukkan 16 digit NIK..." required maxlength="16" value="{{ old('nik') }}" />
                ...
                <x-input label="Nama Lengkap" name="nama" placeholder="Masukkan nama sesuai KTP..." required value="{{ old('nama') }}" />
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-input label="Tempat Lahir" name="tempat_lahir" placeholder="Masukkan kota lahir..." required value="{{ old('tempat_lahir') }}" />
                <x-input label="Tanggal Lahir" name="tanggal_lahir" type="date" required value="{{ old('tanggal_lahir') }}" />
                ...
                <div>
                    ...
                    <select id="agama_id" name="agama_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                        ...
                        @foreach ($agamas as $agama)
                            <option value="{{ $agama->id }}" {{ (int)old('agama_id') === $agama->id ? 'selected' : '' }}>{{ $agama->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="golongan_darah_id" name="golongan_darah_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                        ...
                        @foreach ($goldars as $g)
                            <option value="{{ $g->id }}" {{ (int)old('golongan_darah_id') === $g->id ? 'selected' : '' }}>{{ $g->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="status_kawin_id" name="status_kawin_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                        ...
                        @foreach ($kawins as $kawin)
                            <option value="{{ $kawin->id }}" {{ (int)old('status_kawin_id') === $kawin->id ? 'selected' : '' }}>{{ $kawin->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="kewarganegaraan_id" name="kewarganegaraan_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                        ...
                        @foreach ($wargas as $w)
                            <option value="{{ $w->id }}" {{ (int)old('kewarganegaraan_id') === $w->id ? 'selected' : '' }}>{{ $w->nama }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select id="pendidikan_id" name="pendidikan_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                        ...
                        @foreach ($pendidikans as $p)
                            <option value="{{ $p->id }}" {{ (int)old('pendidikan_id') === $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="pekerjaan_id" name="pekerjaan_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                        ...
                        @foreach ($pekerjaans as $pe)
                            <option value="{{ $pe->id }}" {{ (int)old('pekerjaan_id') === $pe->id ? 'selected' : '' }}>{{ $pe->nama }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div>
            ...
            <div class="space-y-4">
                <x-input label="Alamat Lengkap" name="alamat" placeholder="Masukkan nama jalan, nomor rumah, dll..." required value="{{ old('alamat') }}" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select id="dusun_id" name="dusun_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                            ...
                            @foreach ($dusuns as $d)
                                <option value="{{ $d->id }}" {{ (int)old('dusun_id') === $d->id ? 'selected' : '' }}>{{ $d->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="rw_id" name="rw_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                            <option value="">Pilih</option>
                            @foreach ($rws as $rw)
                                <option value="{{ $rw->id }}" {{ (int)old('rw_id') === $rw->id ? 'selected' : '' }}>RW {{ $rw->nomor }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="rt_id" name="rt_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                            <option value="">Pilih</option>
                            @foreach ($rts as $rt)
                                <option value="{{ $rt->id }}" {{ (int)old('rt_id') === $rt->id ? 'selected' : '' }}>RT {{ $rt->nomor }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="status_tinggal_id" name="status_tinggal_id" required class="block w-full rounded border border-slate-300 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none sm:text-sm">
                            ...
                            @foreach ($tinggals as $tinggal)
                                <option value="{{ $tinggal->id }}" {{ (int)old('status_tinggal_id') === $tinggal->id ? 'selected' : '' }}>{{ $tinggal->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <x-input label="Nomor HP (WhatsApp)" name="nomor_hp" placeholder="Contoh: 08123..." value="{{ old('nomor_hp') }}" />
                <x-input label="Alamat Email" name="email" type="email" placeholder="Contoh: nama@domain.com" value="{{ old('email') }}" />
                
                <div>
                    <label for="foto_file" class="block text-sm font-semibold text-slate-700 mb-2">Foto Profil Penduduk</label>
        ...
        <div class="flex justify-end gap-2 pt-4 border-t border-slate-100">
            ...
            <button type="submit" class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded bg-indigo-600 text-white hover:bg-indigo-700">Simpan Data Penduduk</button>
        </div>
    </form>
</x-card>
@endsection
