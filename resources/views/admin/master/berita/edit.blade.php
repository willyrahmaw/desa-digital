...
@section('breadcrumb-item', 'Edit Berita')
@section('page-title', 'Edit Berita & Artikel Desa')
...
@section('content')
<form action="{{ route('admin.master.berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                <h2 class="text-sm font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Edit Konten Berita / Artikel</h2>
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="judul" name="judul" value="{{ old('judul', $berita->judul) }}" required placeholder="Ketik judul berita menarik..."
                ...
                <div>
                    ...
                    <textarea id="editor" name="konten" rows="12">{{ old('konten', $berita->konten ?? $berita->isi) }}</textarea>
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm sticky top-20">
                ...
                <div class="mb-4">
                    ...
                    <select id="status" name="status" required
                            class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-900 focus:border-indigo-600 focus:outline-none">
                        <option value="Publikasi" {{ old('status', $berita->status) == 'Publikasi' || old('status', $berita->status) == 'published' ? 'selected' : '' }}>Publikasi Langsung</option>
                        <option value="Draft" {{ old('status', $berita->status) == 'Draft' || old('status', $berita->status) == 'draft' ? 'selected' : '' }}>Simpan Sebagai Draft</option>
                    </select>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <input type="date" id="tanggal_publikasi" name="tanggal_publikasi" value="{{ old('tanggal_publikasi', $berita->tanggal_publikasi ?? now()->toDateString()) }}"
                           class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-900 focus:border-indigo-600 focus:outline-none">
                </div>

                {{-- Upload Cover Image --}}
                <div class="mb-6" x-data="{ preview: '{{ ($berita->gambar || $berita->cover_image) ? asset('storage/' . ($berita->gambar ?: $berita->cover_image)) : '' }}' }">
                    ...
                    <p class="text-[10px] text-slate-400 mt-1">Biarkan kosong jika tidak ingin mengubah foto sampul.</p>
                </div>
