...
@section('breadcrumb-item', 'RW')
@section('page-title', 'Daftar Rukun Warga (RW)')
...
@open-edit-modal.window="showEditModal = true; editId = $event.detail.id; editDusunId = $event.detail.dusun_id; editNomor = $event.detail.nomor"
@open-delete-modal.window="showDeleteModal = true; deleteId = $event.detail.id">

    <!-- Table Card -->
    <x-card class="overflow-hidden p-0">
        <div class="overflow-x-auto p-4">
            <table id="rwTable" class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr class="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">
                        <th class="px-6 py-3">No</th>
                        <th class="px-6 py-3">Nomor RW</th>
                ...
                <tbody class="divide-y divide-slate-200 text-sm">
                    @forelse ($rws as $index => $rw)
                        <tr>
                            <td class="px-6 py-4 text-slate-500">{{ $rws->firstItem() + $index }}</td>
                            <td class="px-6 py-4 text-slate-900 font-medium">RW {{ $rw->nomor }}</td>
                            <td class="px-6 py-4 text-slate-700">{{ $rw->dusun->nama ?? '-' }}</td>
                            <td class="px-6 py-4 text-right space-x-2">
                                <button @click="$dispatch('open-edit-modal', { id: '{{ $rw->id }}', dusun_id: '{{ $rw->dusun_id }}', nomor: '{{ $rw->nomor }}' })" 
                                ...
                                <button @click="$dispatch('open-delete-modal', { id: '{{ $rw->id }}' })" 
    ...
    <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60" style="display: none;">
        <div @click.outside="showCreateModal = false" class="bg-white border border-slate-200 rounded-lg p-6 max-w-md w-full">
            <h3 class="text-base font-bold text-slate-900 mb-4">Tambah RW Baru</h3>
            <form action="{{ route('admin.master.rw.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                ...
                <x-input label="Nomor RW" name="nomor" placeholder="Contoh: 01, 002, dll." required />
    ...
    <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60" style="display: none;">
        <div @click.outside="showEditModal = false" class="bg-white border border-slate-200 rounded-lg p-6 max-w-md w-full">
            <h3 class="text-base font-bold text-slate-900 mb-4">Edit Data RW</h3>
            <form :action="'{{ url('admin/master/rw') }}/' + editId" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <x-input label="Nomor RW" name="nomor" x-model="editNomor" required />
                <div class="flex justify-end gap-2 pt-2">
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60" style="display: none;">
        <div @click.outside="showDeleteModal = false" class="bg-white border border-slate-200 rounded-lg p-6 max-w-sm w-full">
            ...
            <p class="text-sm text-slate-500 mb-6">Apakah Anda yakin ingin menghapus data RW ini? Seluruh data RT di bawah RW ini juga akan terhapus.</p>
            <form :action="'{{ url('admin/master/rw') }}/' + deleteId" method="POST">
                @csrf
                @method('DELETE')
